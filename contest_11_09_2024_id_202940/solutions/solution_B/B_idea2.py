...
class Barrier:
    def __init__(self):
        self.__B = [[False] * 8 for _ in range(8)]
...
class Snake(Barrier):
    ...
    def run(self, direction: str, apple: bool = False):
        new_head = new_coor(self.head, direction)
    ...
    for order in permutations(apples):
        ...
        for apple in order:
            ...
            if dist == float('inf'):  # Agar yo'l topilmasa
                ...
                break
            total_dist += dist
            snake.run('r', apple=True)  # Ilonni olma joylashgan nuqtaga olib boramiz
